...
def get_score(board):
    ...
    return - (horizontal_score + diag1_score + diag2_score) * 1000
...
def main():
    ...
    while running:
        ...
        display_score = 0 if app_state == "SOLVED" else current_score / 1000

        score_color = GREEN if display_score == 0 and app_state != "SETUP" else BLACK
        score_text = info_font.render(f"Skóre: {display_score:.0f}", True, score_color)
